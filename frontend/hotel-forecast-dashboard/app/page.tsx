...
export default function Home() {
  ...
  const active    = days.find(d => d.key === selectedDay) ?? days[1]
  const activeData = useMemo(() => forecast?.predictions ?? [], [forecast])

  useEffect(() => {
    let cancelled = false
    const apiDate = selectedDay === "tomorrow-locked" ? "today" : selectedDay
  ...
  // KPIs del día activo
  const totalArrivals   = activeData.reduce((acc, r) => acc + r.arrivals,   0)
  const totalDepartures = activeData.reduce((acc, r) => acc + r.departures, 0)
  const totalDemand     = activeData.reduce((acc, r) => acc + r.demand,     0)
  const totalStaff      = activeData.reduce((acc, r) => acc + r.staff,      0)
  const peakHour        = activeData.length > 0
    ? activeData.reduce((max, r) => r.demand > max.demand ? r : max)
    : { hour: "--:--", demand: 0 }
  ...
  // label dinámico de fecha
  const baseDate = new Date(now)
  const dayOffset = { yesterday: -1, today: 0, tomorrow: 1, dayafter: 2 }
  baseDate.setDate(baseDate.getDate() + (dayOffset[selectedDay as keyof typeof dayOffset] ?? 0))
      ...
      <main className="flex-1 flex flex-col overflow-hidden p-6 gap-4 min-w-0">
        ...
        <div className="flex items-center justify-between flex-shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded" style={{ background: "#C8102E", color: "white" }}>
                Live
              </span>
              <span className="text-gray-400 text-xs">
                {now.toLocaleString("es-MX", { weekday: "long", year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })}
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-sm border border-gray-100">
            <svg width="24" height="24" viewBox="0 0 42 42" fill="none">
              ...
              <text x="21" y="26" textAnchor="middle" fill="#C8102E" fontSize="11" fontWeight="800" fontFamily="Georgia, serif">TCA</text>
            </svg>
            <div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                boxShadow:  tab === t.key ? "0 2px 8px rgba(200,16,46,0.3)" : "none",
              }}
            >
              {t.label}
            </button>
          ...
          <div className="flex flex-col gap-4 flex-1 overflow-hidden min-h-0">
                  ...
                  {days.map(d => {
                    ...
                    return (
                      <div key={d.key} className="relative group">
                        <button
                          onClick={() => !isLocked && selectDay(d.key)}
                          disabled={isLocked}
                          className="px-3 py-1 rounded-lg text-xs font-bold transition-all flex items-center gap-1"
                          style={{
                            background: isSelected ? "#C8102E" : isLocked ? "transparent" : "transparent",
                        ...
                        {/* Tooltip al hover */}
                        {/* Tooltip ABAJO en vez de arriba */}
                        {isLocked && (
                          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden group-hover:block z-10 pointer-events-none">
                            ...
                            <div className="bg-gray-800 text-white text-[10px] rounded-lg px-3 py-2 whitespace-nowrap shadow-lg text-center leading-relaxed">
                              ...
                              <span style={{ color: "#fca5a5" }}>Disponible a las 20:00 hrs</span>
                                ...
                                {/* Carousel — pass currentHour only when viewing today */}
            ...
            <div className="grid grid-cols-5 gap-3 flex-shrink-0">
              ...
              <div className="p-3 rounded-xl bg-white border border-blue-100 shadow-sm">
                ...
                <p className="text-2xl font-black text-blue-700">{totalDemand}</p>
              ...
              <div className="p-3 rounded-xl text-white shadow-sm" style={{ background: "linear-gradient(135deg, #C8102E, #a00d24)" }}>
        ...
        {tab === "charts" && (
          <div className="flex flex-col gap-4 flex-1 overflow-hidden min-h-0">
            ...
            <div className="grid grid-cols-2 gap-4 flex-1 min-h-0">
              <OperationsChart title="Expected Check-ins"  data={checkinData}  dataKey="value" color="#16a34a" />
              <OperationsChart title="Expected Check-outs" data={checkoutData} dataKey="value" color="#C8102E" />
            </div>
          </div>
        )}
