...
IMPORTANTE:
Los modelos fueron entrenados con demanda por personas:
carga_total = sum(h_num_per en llegadas) + sum(h_num_per en salidas)

No se debe usar conteo de reservas.
"""
...
def _load_parquet() -> pd.DataFrame:
    ...
    raise FileNotFoundError(
        "No se encontro hothsp.parquet. Verifica la ruta o la conexion a Azure ML."
    )
...
def _get_hourly_series() -> pd.DataFrame:
    """
    Convierte el parquet en una serie horaria.

    Columnas:
    - arrivals: personas que llegan por hora
    - departures: personas que salen por hora
    - carga_total: arrivals + departures

    Esta definicion debe coincidir con los modelos pkl.
    """
    ...
    df["h_fec_lld"] = df["h_fec_lld"].astype(str).str.strip()
    df["h_hra_lld"] = df["h_hra_lld"].astype(str).str.strip().str.zfill(6)

    df["h_fec_sda"] = df["h_fec_sda"].astype(str).str.strip()
    df["h_hra_sda"] = df["h_hra_sda"].astype(str).str.strip().str.zfill(6)
    ...
    print(
        f"[data_loader] Serie temporal: {len(series)} horas "
        f"({min_date} -> {max_date})"
    )
...
def get_lags_for_date(target_date: str) -> dict:
    """
    Devuelve los valores historicos para las 24 horas de target_date.
    """
    ...
    for h in range(24):
        ...
        result["expected_arrivals"][h] = _get_arrivals(
            series,
            target_ts - timedelta(hours=24),
        )
        result["expected_departures"][h] = _get_departures(
            series,
            target_ts - timedelta(hours=24),
        )
...
def _get_arrivals(series: pd.DataFrame, ts) -> float:
    ...
    subset = series[series.index.hour == ts.hour]["arrivals"]
...
def _get_departures(series: pd.DataFrame, ts) -> float:
    ...
    subset = series[series.index.hour == ts.hour]["departures"]
    return float(subset.mean()) if len(subset) > 0 else 0.0